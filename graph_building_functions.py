...
def distance_graph(features, distance='Euclidean', thre=0.):
    ...
    adj = distance_each_other.distance(features, features, distance_mode=distance, if_similarity=True)
    adj = torch.where(adj > thre, adj, zeros)
    return adj
...
def k_neighbors_graph(features, k):
    distance_matrix = distance_each_other.distance(features, features)
    distance_arg = torch.argsort(distance_matrix, dim=1)[:, :k]
    adj = torch.zeros((len(features), len(features)))
    ones = torch.ones((len(labels), len(labels)))
    zeros = torch.zeros((len(labels), len(labels)))
    for i in range(len(distance_arg)):
        for j in range(k):
            adj[i, distance_arg[j]] = 1
    adj = torch.where(distance_matrix >= 0, ones, zeros)
    return adj
